...
class MultiHeadSelfAttention:
    ...
    def __call__(self, x, mask=None):
        ...
        Q = Q.reshape(B, T, self.num_heads, self.d_head).transpose(1, 2)  # (B, num_heads, T, d_head)
        K = K.reshape(B, T, self.num_heads, self.d_head).transpose(1, 2)  # (B, num_heads, T, d_head)
        V = V.reshape(B, T, self.num_heads, self.d_head).transpose(1, 2)  # (B, num_heads, T, d_head)
        
        # Compute scaled dot-product attention scores
        # Q @ K^T / sqrt(d_head)
        scores = Q.matmul(K.transpose(-2, -1)) / (self.d_head ** 0.5)  # (B, num_heads, T, T)
        ...
        attn_weights = scores.softmax(axis=-1)  # (B, num_heads, T, T)
        
        # Apply attention weights to values
        out = attn_weights.matmul(V)  # (B, num_heads, T, d_head)
